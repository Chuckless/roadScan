	...
	function draw(begin, end,  map){
		if(begin != end){
			...
			var points, manyDifferentTimesm, holePoints;
			...
			if(manyDifferentTimes >= MIN_DIF_TIMES){
				...
				console.log("DIFERENT: " + manyDifferentTimes);
				
				IRIColorDraw(Path, points, manyDifferentTimes, map);
				console.log("---------------------");
				lineNumber++;
			}else{
	...
	function perpendicularVetor(begin, end, map){
	    ...
	    //console.log("distance " + distance);
	      ...
	      var polygonCoords = [point , point2, point4, point3, point];
	...
	// ############################################################# //
	function IRIColorDraw(Path, points, manyDifferentUsers, map){
		var GOOD_IRI = 5;
		var BAD_IRI = 8;
		var quality;
		var brng = points[0].getAttribute('brng');

		var IRIMean = getMean2(points);
		...
		setInfoWindow(map, quality, Path, infoWindow, manyDifferentUsers, brng);
	}
	...
	// ############################################################# //
	function setInfoWindow(map, quality, thisPath, infoWindow, manyDifferentUsers, brng){
	  ...
	  if(manyDifferentUsers >= MIN_DIF_TIMES && manyDifferentUsers <= 5)  {
	    toolTip = toolTip + 
	    '<img src="lilgoldstar.png">';
	  }else if(manyDifferentUsers > 5 && manyDifferentUsers <= 10){
	    toolTip = toolTip + 
	    '<img src="lilgoldstar.png">' +
	    '<img src="lilgoldstar.png">' +
	    '<img src="lilgoldstar.png">';
	  }else if(manyDifferentUsers > 10){
	    toolTip = toolTip + 
	    '<img src="lilgoldstar.png">' +
	    '<img src="lilgoldstar.png">' +
	    '<img src="lilgoldstar.png">' +
	...
	function getMean2(points){
		...
		for(var i = 0; i < points.length; i++){
			stdv.push(parseFloat(points[i].getAttribute("IRI")))
			speed.push(parseFloat(points[i].getAttribute("speed")));
			flags.push(parseInt(points[i].getAttribute("flag")));
		}
		

		return(flagCount(flags));
		//return(flags[0]);


			/*downloadUrl2("callR.php", stdv, speed, function(data) {
				var xml = data.responseXML;
				Rreturn = xml.documentElement.getElementsByTagName("return");
				finalValue = Rreturn[0].getAttribute("stdv");
			});*/
	    //alert(finalValue);
	}
	 // ############################################################### //
	 function flagCount(flags){
	 	...
	 	return parseInt(sum/flags.length);
	...
	function downloadUrl2(url, stdv, speed, callback) {
		var request = window.ActiveXObject ? new ActiveXObject('Microsoft.XMLHTTP') : new XMLHttpRequest;
		request.onreadystatechange = function() {
			if (request.readyState == 4) {
				request.onreadystatechange = doNothing;
		...
		request.send('stdv='+stdv +'&speed=' + speed);
	}
